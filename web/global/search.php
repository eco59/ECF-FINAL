...
if ($recupJeuxVideos->rowCount() > 0) {
    while ($row = $recupJeuxVideos->fetch(PDO::FETCH_ASSOC)) {
        ...
        echo "<td class='titre'><a href='../details/jeux_detail.php?id=" . htmlspecialchars($row['id']) . "'>" . htmlspecialchars($row["titre"]) . "</a></td>";
        ...
        $imagePath->execute([htmlspecialchars($row['id'])]);
        ...
        echo "<td class='contenu'>" . htmlspecialchars($row["description"]) . "</td>";
