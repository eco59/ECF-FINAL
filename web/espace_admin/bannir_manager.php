<?php
    ...
    if(isset($_GET['id']) AND !empty($_GET['id'])){
        ...
        $recupManager = $bdd->prepare('SELECT * FROM manager WHERE id =?', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
        $recupManager->execute(array($getid));
        if($recupManager->rowCount() >0){
            // On supprime dans la base de donnée
            $bannirManager = $bdd->prepare('DELETE FROM manager WHERE id = ?', array(PDO::ATTR_CURSOR => PDO::CURSOR_FWDONLY));
